...
class ConversationGraphBuilder:
    ...
    def route_decision(self, state: ConversationState) -> str:
        ...
        if "NONE" in response.upper() or len(response) > 200:
            ...
            # Store the question in state for the clarify_node to use
            # We use a little trick: return it via a state update in router_node
            # Actually we handle this by storing in plan temporarily
            state["plan"]["_pending_clarification"] = response
            return "clarify"
    ...
    def clarify_node(self, state: ConversationState) -> dict:
        ...
        """
        plan = state["plan"]
        question = plan.get("_pending_clarification", "Could you provide more details?")
